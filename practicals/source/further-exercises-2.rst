...
NEMO diagnostic files are named ``nemo_<workflow-name>o*grid_[TUVW]*``. 
SI3 diagnostic files are named ``si3_<workflow-name>io*icemod*``. To see what files are produced, run: ::
...
The ocean and ice model components start with single start files (see for example, :guilabel:`si3->Restart files`), but notice in ``share/data/History_Data/NEMOhist``, the model has written multiple start files. Type: ::
